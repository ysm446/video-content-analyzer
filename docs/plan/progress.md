...
主要機能（字幕生成・翻訳・2言語プレイヤー・動画分析/チャプター・チャット）はすべて動作する状態。
直近1週間で以下を実施済み:

- 動画読込ボタンを操作エリアから削除し、SRTの読込・解除をプレイヤーの「字幕」メニューへ移動（2026-09-12）

- 処理ごとのオプション行を実装（2026-09-12）。保存・復元、1行展開、処理中ロック、文字起こし設定失敗時の復元を Electron＋モックAPI で検証。実モデル推論は未実施（→ design/process-options.md）

- レポートのブックマーク掲載・画像抽出を廃止（2026-09-12）。既存書き出しは再生成で反映

- LLM 一覧の mmproj 照合を修正（2026-09-12）。複数量子化版の画像対応モデル14件の欠落を解消。回帰テスト4件成功、実モデルのロードは未確認

- アプリ全体のレビューと修正・最適化（2026-09-12 → changelog）。処理中の動画切り替えによる
  結果の混入、100 分以降の字幕が章立てから消える問題、外部リンクによるウィンドウ遷移、
  llama-server のエラー行の取りこぼし、レポート画像選定の O(n²)、モデルフォルダの
  毎バッチ再スキャン等を修正。フロントは `node --check` と静的確認のみで実機未確認

- ブックマーク追加後のタイトル自動フォーカスと Enter 保存（Shift+Enter 改行・IME 変換中は保存しない、2026-09-10）

- アプリアイコンを動画分解モチーフの3D画像へ更新（PNG・Windows用ICO、2026-09-10）

- 動画分析パイプラインの品質改善（json_schema 構造化出力・コンテキスト予算・
  timestamp スナップ・h:mm:ss バグ修正等 → design/video-analysis-review.md）
- 字幕翻訳の精度改善（先読み文脈・用語集・バッチ翻訳）と quality/fast モード
  （→ design/translation-accuracy.md）
- チャットの高速化（サムネール再利用・並列シーク抽出）・マルチターン・
  テンプレート質問チップ（→ design/qa-chat.md）
- 設定のポップアップ化・ランタイム管理（llama.cpp/Whisper/ffmpeg のインストールと切り替え）
- UI デザインの統一（トークン化 → design/style-guide.md）
- シーンサムネールのサーバー側生成（重複・前チャプター混入の修正）
- ファイルマネージャー（ルートフォルダ方式の動画一覧サイドバー・2026-07-18）
- プレイヤー UI 改善（コントロール2段化・シークバー全幅＋チャプター/ブックマークマーカー・
  ホバープレビュー・2026-07-18）
- 動画レポート（章立て＋代表画像の 1 ページ Markdown/HTML 書き出し。VL による章本文生成・
  画像選定まで実装・2026-09-10）
- 動画の種類プリセットと字幕主導の話題アウトライン（分析・レポート共通・2026-09-10）

**未実施の共通事項: 実動画・実モデルでの通し確認**（各節の未チェック項目と
[plan.md](plan.md) の「実機確認」を参照）。

## レポートの内容再構成（2026-09-12 → design/report.md）
- [x] `generate_synthesis`（要点・テーマ別・主要な事実）と形式設定 `report_layout`
- [x] md / html / report.json / プレビューの 4 か所を揃える。字幕だけでもレポート生成可
- [ ] 実動画・実モデルでの品質確認（テーマの粒度・times の精度・分析なしでの生成）

## レポートのモーダル表示（2026-09-12）
- [x] レポートタブを廃止し「レポートを表示」ボタン＋大きなモーダルに変更
- [ ] Electron 実機での表示確認（画像グリッド・スクロール・クリックでシーク→閉じる）

## アプリ全体レビュー（2026-09-12 → changelog）
- [x] バックエンド: cancel の状態管理・llama-server エラー行・タイムスタンプ 100 分超・
      レポート画像の差し替え／類似判定の高速化・スキャンキャッシュ・イベントループのブロック解消
- [x] フロント: 動画切り替え中の結果混入の防止（世代番号・開始時の動画を記憶）・SSE 切断検知・
      チャット描画のフレーム単位化・サムネールのブラウザキャッシュ・外部リンク／IPC の安全化
- [x] バックエンドの単体スモークと実起動での API 確認
- [ ] Electron 実機での通し確認（動画オープン→分析→切り替え→チャット→ブックマーク→レポート、
      チャット回答内リンクが既定ブラウザで開くこと、サムネールが再生成後に更新されること）

---

## 動画の種類と話題主導の章立て（2026-09-10 → design/video-kinds.md）
- [x] `backend/outline.py`（プリセット・アウトライン生成）と `VideoReviewer.text_infer`
- [x] 動画分析への組み込み（`video_kind`・`outlining` イベント・meta に種類と基準を保存）
- [x] レポートの章立て作り直し（`rebuild_chapters`・シーン検出の共用）
- [x] 設定 UI（動画の種類・作り直しチェック）
- [x] 実動画（Computex 12 分）で分析 6 章・レポート 5 章を確認
- [ ] auto の判定を場面変化の頻度・発話密度で presentation / talk に分ける
- [ ] 対談・チュートリアル・映像作品の実動画での確認
- [ ] Electron 実機での設定 UI と分析→チャプター表示の通し確認

## 動画レポート（2026-09-10 第1段階完了 → design/report.md）
- [x] `backend/report.py`: シーン変化検出＋知覚ハッシュ間引き＋1280px 抽出＋ Markdown/JSON/単一 HTML 書き出し
- [x] `POST /report/generate`（SSE）・「レポート」ボタン・`_report/` のサイドカー扱い
- [x] 合成動画（TestClient）と実動画（Computex 12 分）で生成確認
- [ ] Electron 実機でボタン→進捗→エクスプローラー表示の通し確認
- [x] 第2段階: VL による章ごとの本文生成と画像選定（2026-09-10。設定で ON/OFF・画像上限）
- [x] 実動画＋ロード済み VL モデル（Qwen3.8-27B）で 19 章 116 秒の通し確認
- [ ] 長尺動画（1 時間・50 章以上）での所要時間と本文品質の確認
- [x] 第3段階: アプリ内プレビュー（レポートタブ・クリックでシーク・2026-09-10）
- [ ] 第3段階の残り: 章単位の再生成・複数ページ化オプション・解像度の設定 UI
- [ ] レポート・チャプターの世代保持（再生成・再分析で上書きされる問題。→ plan.md「生成物のバージョン管理」）
- [ ] Electron 実機でレポートタブの表示・シーク・再生成の確認

## ファイルマネージャー（左サイドバーの動画一覧・2026-07-18 完了）
- [x] 行の右クリックで「…」メニューを開く（2026-09-10）
- [x] モデル管理ポップアップで最近使ったモデルを先頭グループに表示・項目クリックで即ロード（2026-09-10）
- [x] ポート使用中の自動切り替え（backend / llama-server。2026-09-10。Electron 実機での確認は未実施）
- [x] `POST /folder/list`（直下のみ・遅延読み込み）/ `POST /folder/search`（再帰・部分一致・上限300件）
- [x] `*.cache/`・`*_screenshot/`・隠しフォルダは一覧から除外（サイドカー方針）
- [x] 左サイドバー: ツリー表示・分析/字幕バッジ・サムネール・検索（フラット表示）・
      再生中ハイライト・トップバーで表示切り替え
- [x] `root_folder` / `show_file_panel` を ui-settings で永続化、`dialog:openFolder` IPC 追加
- [x] 処理完了時（文字起こし/補正/翻訳/保存）に親フォルダのバッジを自動更新
- [x] 幅のドラッグリサイズ（`file_panel_width` 永続化・ダブルクリックで既定幅）
- [x] 行の「…」メニュー: 名前の変更（`POST /file/rename`・サイドカー追随・
      data.json の video 更新・インライン編集）とごみ箱への移動（`shell.trashItem` IPC・
      動画はキャッシュ/字幕/スクリーンショットもまとめて移動）
- [x] 開いている動画のリネーム・削除はロック回避のため先にクローズ（リネーム後は自動再オープン）
- [x] エンドポイント・永続化・リネーム E2E は実サーバー＋テスト用フォルダで curl 検証済み
- [ ] Electron 実機での表示・操作確認は未実施
- [ ] 将来候補: タグ・ジャンルの横断検索（第2段階）

## プレイヤーコントロール2段化・シークバーマーカー（2026-07-18 完了）
- [x] コントロールを2段レイアウトに（1段目: シークバー全幅、2段目: 操作ボタン列）
- [x] シークバー上にチャプター（灰）・ブックマーク（橙）のティックマーカーを重ね描画
      （分析・チャプター編集・ブックマーク編集・動画ロードで再描画）
- [x] コントロール列の「マーカー」トグルで表示切り替え（`seek_markers` を ui-settings に永続化・既定 ON）
- [ ] Electron 実機での表示・操作確認は未実施
...
- [x] ファイル名 `{動画名}_{HH-MM-SS.mmm}.{png|jpg}`、末尾/超過時刻のクランプ
- [x] 設定ポップアップに「プレイヤー」セクション追加（保存形式 png/jpg、ui-settings 永続化）
- [x] 合成動画＋TestClient でエンドポイント経由のテスト済み
- [ ] 実アプリ（Electron）で F12 押下の動作確認は未実施

## シーンサムネールのサーバー側生成（2026-07-04 完了）
- [x] 重複・前チャプター混入の原因特定（seeked とフレーム描画のレース）
- [x] POST /cache/thumbnails/generate（ffmpeg 入力シーク・並列4・480px）
- [x] フロントの canvas キャプチャを置き換え、/cache/image に no-store 付与
- [x] 合成動画での実生成テスト済み
- [ ] 実動画での確認（分析を一度実行してサムネールが正しいか）は未実施

## チャットのテンプレート質問チップ（2026-07-04 完了）
- [x] 固定テンプレート（要約・重要ポイント）は即表示
- [x] 内容ベースの質問は /review/questions で遅延生成（未ロード時は固定のみ、
      回答後は会話を踏まえたフォローアップに更新、キャッシュには保存しない）
- [x] チップの配置を入力欄上の常時表示から、会話履歴の末尾（最後の回答の直後）へ変更
      （2026-09-10。回答生成中は非表示）
- [ ] 実モデルでの質問品質の確認は未実施

## UI スタイルガイド（2026-07-03 明文化 → 2026-08-11 統合）
- [x] デザインルールを `docs/design/style-guide.md` に明文化
      （旧 `ui-design-guidelines.md` を統合・改題。UI に関する基準はこの1ファイルに一本化）
- [x] トークン化・意味色/青系/フォーカスの統一・絵文字除去・文字サイズ/角丸の是正を実装
      （トークン・フォーカス・バッジは common.css に集約済み）
- [x] レイアウト・コンポーネント規約・実装ルール・変更時チェックリストを追記（2026-08-11）
- [x] CLAUDE.md / AGENTS.md に「UI 変更時は style-guide.md に従う」ルールを明記（2026-08-11）
- [ ] 残: インラインスタイルのクラス抽出と common.css 分離、ライトテーマ、
      文字サイズ逸脱（ファイルツリーの 12.5/10.5/10px）の是正
...
## モデルフォルダの指定（2026-08-23 完了）
- [x] `settings.json` の `models_dir`（空文字＝既定の `models/`）でスキャン先を切り替え
- [x] `backend/model_catalog.py` を `models_dir()` 解決に変更（フォルダ直下の GGUF も検出）
- [x] `POST /runtime/models-dir`（バリデーション＋選択中モデルの再同期）／
      `GET /runtime/status` に `models_dir` を追加
- [x] 設定 → ランタイムに「モデルフォルダ」行（パス表示・GGUF 数・選択・既定に戻す）
- [ ] Electron 実機での通し確認（外部フォルダ選択→モデル一覧更新→ロード→推論）は未実施
