...
## 2026-07-04
- **チャットにテンプレート質問チップを追加（YouTube AI チャット風）**
  - 入力欄の上に右寄せのチップを表示。固定2種（「この動画を要約して」
    「重要なポイントを3つ挙げて」）＋動画分析で生成した内容ベースの質問（最大3つ）
  - 内容ベースの質問は分析 JSON に `questions` フィールドを追加して同時生成
    （json_schema に組み込み、出力コスト増は数十トークン）。キャッシュの
    meta.questions に保存され、分析済み動画では次回オープン時も表示される
  - チップをクリックすると即送信。回答生成中はクリック無効
