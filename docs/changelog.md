...
## 2026-09-12
- **操作エリアから動画・字幕の読込ボタンを整理**: 動画読込をファイル一覧・ドラッグ＆ドロップに集約。原文／日本語SRTの読込・解除はプレイヤーの「字幕」メニューへ移動し、表示切替と同じ場所で操作できるように変更
- **分析操作を処理ごとの設定行に整理**: 文字起こし・補正・字幕生成・動画分析・レポートを「処理名 / 設定要約 / 実行」で表示。オプションは1行だけ展開、選択を保存・復元、処理中は設定固定。文字起こしは取得済みモデルとエンジンを選択。Electron＋モックAPIで操作・保存失敗・380px幅を検証（→ design/process-options.md）
- **レポートからブックマークを除外**: Markdown・HTML・JSON・アプリ内プレビューの掲載と画像抽出を削除。レポート生成リクエストでも送信しない。プレイヤーのブックマークは従来どおり保持
- **LLM 一覧で画像対応モデルの量子化違いが欠落する問題を修正**
  - mmproj の BF16/F16 と本体の Q4_K_M/Q6_K などの末尾を除いて同一モデル名を照合。複数量子化版でも共通 projector を検出
  - 別モデルへの誤対応を避け、正規化後は完全一致。汎用名 projector のフォールバックは本体1件の場合のみ維持
  - 回帰テスト4件成功。設定済みフォルダの検出は全30件・画像対応12→26件。実モデルのロード確認は未実施
- **レポートを内容から再構成（要点・テーマ別のまとめ・主要な事実）**
  - 章ごとの本文生成の後に、全章のまとめ＋字幕サンプルからテキストのみ推論で
    `key_messages` / `themes[{title, body, times}]` / `key_facts` / `summary` / `genre` / `tags` を生成
    （`report.generate_synthesis`・SSE `synthesizing`）。テーマには章の画像から時刻が最も近いものを流用
  - 設定 → 動画分析 → レポート に「形式」（要点・テーマ別＋時系列の章 / 要点・テーマ別のみ /
    時系列の章のみ。ui-settings `report_layout`・既定 both）。md / html / report.json /
    アプリ内プレビューの構成を揃え、テーマの関連時刻・画像のクリックでシーク
  - 分析結果のチャプターを必須にしない: 字幕があれば「レポート」ボタンが有効になり、章立ての作り直し
    と再構成で分析なしのレポートを作れる（meta が無ければ再構成の概要・ジャンル・タグを使う）
  - 検証は再構成〜md/html 生成のモック単体テストのみ。実モデルでの品質確認は未実施（→ plan.md）
