...
| チャプターエリア | ヘッダーのタブで「チャプター」（LLM 生成シーン一覧・下書き→保存方式）・「ブックマーク」を切り替え |
| レポートモーダル | 分析パネル操作行の「レポートを表示」（生成済みのときのみ有効）で開く大きなポップアップ（`#reportModal`・設定モーダルと同じ暗転＋ぼかし、幅 1180px / 高さ 960px 上限）。`POST /report/load` で読んだ report.json を構造から描画し、章タイトル・目次・画像のクリックで再生位置へシークしてモーダルを閉じる。ヘッダーに「場所を開く」「再生成」「×」。Esc / 背景クリックで閉じる。生成完了時に自動で開く |
