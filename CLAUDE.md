...
動画ファイル
  ...
  → (エンジン=whisperx のとき) [backend/align.py] wav2vec2 CTC 強制アライメントで
    セグメント時刻を精密化（WhisperX 移植・torch + transformers のみ使用）
...
- `POST /transcribe` — 動画→原文SRT（SSE）。エンジンが whisperx のときは
  faster-whisper 転写後に wav2vec2 強制アライメントで時刻を精密化（backend/align.py）
...
### 動画レポート
- `POST /report/load` — 生成済み `report.json` を返す（`{video_path}` → `{data, report_path, html_path}`。無ければ 404）
- `GET  /report/image?video_path=...&name=images/...` — レポート画像を返す（`images/` 外は 403）
...
- `POST /runtime/asr/engine` — 文字起こしエンジンの切り替え
  （`{engine: "faster-whisper" | "whisperx"}`。settings.json の `asr_engine`。
  whisperx は faster-whisper 転写＋wav2vec2 強制アライメント。整列モデル
...
### `/transcribe`
```
loading_model       → ASR モデルが未ロードの場合のみ
extracting_audio    → 文字起こし開始
loading_align_model → {language, model}（エンジン=whisperx のみ。初回は HF ダウンロード込み）
aligning            → {current, total}（エンジン=whisperx のみ。セグメント単位で進捗更新）
align_warning       → {message}（未対応言語・整列失敗。元の時刻のまま続行し全体は失敗させない）
saving_srt          → {segments}
canceled            → ユーザーが POST /cancel で中断したとき
done                → {srt_path, segments}
error               → {message}
```
...
- `transformers` は WhisperX 方式アライメント（`align.py` の wav2vec2）と `translator.py` の
  HF フォールバック経路で使用（どちらも遅延 import）。バージョンピンは不要
- torch は Blackwell(sm_120) 対応のため **cu130 ホイール**を使う
- **whisperx パッケージは導入禁止**: `torch~=2.8.0` 固定のため pip 導入すると
  cu130 torch が CPU 版にダウングレードされて GPU が壊れる。アルゴリズムは
  `backend/align.py` に移植済み（BSD-2 帰属明記）なのでそちらを使う
- pip 版 cuDNN（CTranslate2 用）と torch 同梱 cuDNN は混在するとクラッシュするため、
  `align.py` の wav2vec2 推論は `torch.backends.cudnn.flags(enabled=False)` で実行
...
| チャプターエリア | ヘッダーのタブで「チャプター」（LLM 生成シーン一覧・下書き→保存方式）・「ブックマーク」・「レポート」（生成済み report.json のアプリ内プレビュー。`POST /report/load` で読み、章タイトル・目次・画像のクリックで再生位置へシーク。ツールバーに「場所を開く」「再生成」、未生成時は「レポートを生成」ボタン）を切り替え |
...
| レポート | 分析パネル操作行の「レポート」ボタン（章立てがあるときのみ有効）。`POST /report/generate` で `{動画名}_report/` に 1 ページ Markdown＋単一 HTML＋画像を書き出し、完了時にレポートタブのプレビューを開く。設定 → 動画分析 → レポート で VL による本文生成の ON/OFF と 1 章あたりの画像上限を指定。進捗はステータスバー（kind=report）、中止ボタン対応 |
...
- 方向性: フラット＋ややデフォルメ。stroke-width は基本 2.2（サイドバー 20px は 2、
  no-video 48px は 1.4、キャレットは 2.4）で統一する
